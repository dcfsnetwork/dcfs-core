...
func (p *Parlia) GetBlockReward(header *types.Header, rewardBlockNumber *big.Int) *big.Int {

	var gwei = big.NewInt(1000000000)

	var base = big.NewInt(4280821918)
	var yearCount = big.NewInt(5256000 * 6)
	if rewardBlockNumber == nil {
		return common.Big0
	}
	if rewardBlockNumber.Cmp(common.Big0) == 0 {
		return common.Big0
	}
	if header.Number.Cmp(rewardBlockNumber) == -1 {
		return common.Big0
	}

	currentRewardCount := new(big.Int).Sub(header.Number, rewardBlockNumber)
	if currentRewardCount.Cmp(yearCount) == -1 {
		return new(big.Int).Mul(base, gwei)
	} else if currentRewardCount.Cmp(yearCount.Mul(yearCount, big.NewInt(2))) == -1 {

		return new(big.Int).Div(new(big.Int).Mul(base, gwei), big.NewInt(2))
	} else if currentRewardCount.Cmp(yearCount.Mul(yearCount, big.NewInt(3))) == -1 {

		return new(big.Int).Div(new(big.Int).Mul(base, gwei), big.NewInt(4))
	} else if currentRewardCount.Cmp(yearCount.Mul(yearCount, big.NewInt(4))) == -1 {

		return new(big.Int).Div(new(big.Int).Mul(base, gwei), big.NewInt(8))
	} else if currentRewardCount.Cmp(yearCount.Mul(yearCount, big.NewInt(5))) == -1 {

		return new(big.Int).Div(new(big.Int).Mul(base, gwei), big.NewInt(16))
	}
	return common.Big0
}
...
func (p *Parlia) trySendBlockReward(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	...
	rewardBlockNumber, err := p.getRewardSwtich(header.ParentHash)
	if err != nil {
		log.Info("rewardSwitch", "error", err)
	}

	block_reward := p.GetBlockReward(header, rewardBlockNumber)
	...
	if block_reward.Cmp(common.Big0) > 0 {
		state.AddBalance(header.Coinbase, block_reward)
	}
	...
	if block_reward.Cmp(common.Big0) > 0 {
		...
		data, err := p.abi[systemcontract.RewardHomeContractName].Pack(method)
		if err != nil {
		...
		msg := types.NewMessage(header.Coinbase, &systemcontract.RewardHomeContractAddr, nonce, block_reward, math.MaxUint64, new(big.Int), data, nil, true)

		if _, err := vmcaller.ExecuteMsg(msg, state, header, newChainContext(chain, p), p.chainConfig); err != nil {
			log.Error("exec distribute failed ", err)
			return err
		}
	}
	return nil
...
func (p *Parlia) getRewardSwtich(blockHash common.Hash) (*big.Int, error) {
	blockNr := rpc.BlockNumberOrHashWithHash(blockHash, false)
	...
	data, err := p.abi[systemcontract.RewardSwitchContractName].Pack(method)
	if err != nil {
		log.Error("Unable to pack tx for getValidators", "error", err)
		return nil, err
	}
	// call
	msgData := (hexutil.Bytes)(data)
	toAddress := systemcontract.RewardSwitchContractAddr
	gas := (hexutil.Uint64)(uint64(math.MaxUint64 / 2))
	result, err := p.ethAPI.Call(ctx, ethapi.CallArgs{
		Gas:  &gas,
		To:   &toAddress,
	...
	var out *big.Int
	if err := p.abi[systemcontract.RewardSwitchContractName].UnpackIntoInterface(&out, method, result); err != nil {
		return nil, err
	}
	return out, nil
